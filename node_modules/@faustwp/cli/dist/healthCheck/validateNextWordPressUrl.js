 ...
 */
export async function validateNextWordPressUrl() {
    ...
    try {
        ...
        if (!response.ok) {
            if (response.status === 404) {
                ...
                warnLog('Route not found: Please update your FaustWP plugin to the latest version.');
            }
            else {
                errorLog('Validation Failed: Your Faust front-end site URL value is misconfigured. It should NOT match the `NEXT_PUBLIC_WORDPRESS_URL.`');
                process.exit(1);
            }
        }
    }
